...
class Pol_Analyzer:
    ...
    def __init__(self, keyword, category, sub_keyword, channel,startDate, endDate, quantity):
    ...
    def pol_anaylze(self):
            ...
            # sql = "select * from bigkinds where keyword=\'%s\' and channelName=\'%s\' and postDate>=\'%s\' and postDate<=\'%s\'" % \
            #       (self.keyword, self.channel, self.startDate, self.endDate)
            sql = "select * from bigkinds where keyword=\'%s\'"%(self.keyword)
            curs.execute(sql)
            rows = curs.fetchall()
            ...
            for i, row in enumerate(rows):
                ...
                #print('phrase:',phrase)
                print('index:',id_fk,'keyword:',keyword,'category:',category,'sub_keyword:',sub_keyword,'point_pos_avg:',point_pos_avg,'point_neg_avg:',point_neg_avg, 'point_neu_avg:',point_neu_avg)
